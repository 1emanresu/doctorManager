	...
	$.ajax({
		...
		success: function(result) {
			if(result != null || result != '') {
				var a = result;
				getRiskModel('div-modelChart', JSON.stringify(a), 'risk_result');
				var list=[];
				for(var i=0;i<a.length-1;i++){
					list.push(a[i].cname);
				}
				var url2=address.replace("doctormanager","knowledge")+"riskModel/getAdvice.do";
				$.ajax({
					type: "post",
					...
					success: function(e) {
						if(e != null || e != '') {
							...
							if(e.length==0){
								$("#abnormal-analysis").append("<div class='notHaveYet'>无</div>");
...
function getRiskModel(ChartID, gradata, showID) {
	...
	var resizemyChart1 = function() {
		...
		document.getElementById('riskChart').style.width = window.innerWidth/3 + 'px';
	};
	//设置容器高宽
	resizemyChart1();
	...
	/*********  指标各项异常信息    **********/
	var tablecont = "<div class='historyTableContainer'><table id='riskfactor-table' class='table-css'><tr><td>序号</td><td>出现时间</td><td>风险因素</td> <td>因素来源</td><td>风险分值</td></tr></table></div>"
	...
	if(advice != null && advice.length > 0) {
		...
		$("#abnormal-analysis").append(tipcont)
	}
...
function creatDiv(ChartID, showID) {
	...
	var divCont = "<div class='row'><div class='col-md-6 riskfactor-container'><label class='fontblod'>风险因素:</label><div class='riskfactor-Msg'>" +
		"</div></div></div>";
	$('#' + showID).append(divCont);
}
drag($('.class_serious_risk'));