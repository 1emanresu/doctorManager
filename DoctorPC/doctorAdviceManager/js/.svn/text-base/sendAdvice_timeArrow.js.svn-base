...
$(function() {
	//按类型查看转换样式改变
    $("#view_by_project a").click(function() {
        $("#view_by_project").css("background-color", "#8CB6EF");
        $("#view_by_project a").css("color", "white");
        $("#view_by_weeks").css("background-color", "#EAEEF1");
        $("#view_by_weeks a").css("color", "#696969");
    });
    $("#view_by_weeks a").click(function() {
        $("#view_by_weeks").css("background-color", "#8CB6EF");
        $("#view_by_weeks a").css("color", "white");
        $("#view_by_project").css("background-color", "#EAEEF1");
        $("#view_by_project a").css("color", "#696969");
    });
    $.ajax({
        type: "post",
...
function modalForWeekly(i) {
    ...
    //按钮下的颜色还原
   /* $(".general-color").css("color","#333");*/
    /*$(".btn-many").each(function(i){
    	if($(this).css("background-color")!='rgba(255, 255, 255, 0)'){
    ...
    if(index == 1) {
    	...
    	$(".first-span").css("color","#8CB6EF");
    	/*$("#btn"+index).css("background-color","#8CB6EF");*/
    ...
    if($("#a-selfTest").parent().attr("class")=="active"){
    	setTimeout(function(){
    		$("#a-selfTest").click();
    	},300);
    };
...
function changeColor(week) {
   ...
   $("#btn1").find("img").attr("src","../Remotelnterrogation/images/1-411.png");
    ...
    if(week > 36) {
    	...
    	modalForWeekly(i);//默认执行当前孕周范围的数据
}
