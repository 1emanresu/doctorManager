	...
	$.ajax({
		...
		success: function(result) {
			if(result != null || result != '') {
				...
				//alert(111);
				getRiskModel1('div-modelChart', JSON.stringify(a), 'risk_result');
				//console.log(data);
			}
		}
	});
 ...
 */
function getRiskModel1(ChartID, gradata, showID) {
	var gdata = JSON.parse(gradata)
	if(flag) {
		creatDiv1(ChartID, showID);
		flag = false;
	}

	//用于使chart自适应高度和宽度,通过窗体高宽计算容器高宽
	var resizemyChart1 = function() {
		document.getElementById('modelChart').style.width = window.innerWidth / 4 + 'px';
		document.getElementById('riskChart').style.width = window.innerWidth / 3.8 + 'px';
	};
	//设置容器高宽
	resizemyChart1();
	...
	/*********  指标各项异常信息    **********/
	var tablecont = "<label>姓名：" + aname + "</label><table id='riskfactor-table' class='riskfactor-table'><tr><td>序号</td><td>出现时间</td><td>风险因素</td> <td>因素来源</td><td>风险分值</td></tr></table>"
...
//生成所需的DIV
function creatDiv1(ChartID, showID) {
