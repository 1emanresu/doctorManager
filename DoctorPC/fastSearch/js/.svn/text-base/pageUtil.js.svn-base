...
function getSelectOption(table, classname) {
	$.ajax({
		type: "post",
		url: UtilAddress + "datadictionary/findByTable.do",
		dataType: "json",
		data: {
			table: table
		},
...
function getSelectOptionByIndex(table, index, classname) {
	...
	$.ajax({
		type: "post",
		url: UtilAddress + "datadictionary/findByTableAndIndex.do",
		dataType: "json",
		data: {
			table: table,
			index: index
	...
	paging: function(option) {
 	...
 	paging2: function(option) {
		...
		obj.find(".page2").click(function() {
			...
			var showPage = _this.text();
			light(_this);
			selectFileVideo((showPage - 1) * 6, 6);
		});
		//下一页
		obj.find("#next2").click(function() {
			...
			selectFileVideo((nextPage - 1) * parseInt(6), 6);
		});
		...
		obj.find("#end2").click(function() {
			...
			showHome(true);
			light(obj.find("span").eq(pageSize + 1));
			selectFileVideo((totalPage - 1) * parseInt(6), 6);
		});
		//刷新显示条数
		obj.click(function() {
			var size = parseInt(6);
