...
function operation(row, tId) {
	...
	if(row.doctor == 1) {
		...
		if(row.vipGradeName == null && row.vipGradeName == undefined){
			htm +="&nbsp;&nbsp;&nbsp;&nbsp;<a style='cursor: pointer;' onclick='toService(" + JSON.stringify(row) + ",3);'>开通服务</a>";
		}
...
function toService(row,i){
	sessionStorage.setItem("row", JSON.stringify(row));
	sessionStorage.setItem("whether2", i);
	location.href = '../../serviceManage/serviceManage.html';
}

//设置mid
function setMid(mid, gname) {
	sessionStorage.setItem("mid", mid);
	sessionStorage.setItem("whether", 2);
