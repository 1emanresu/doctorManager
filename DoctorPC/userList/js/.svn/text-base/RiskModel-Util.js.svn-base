...
//var UtilAddress = address.substring(0,find(this.address,'/',2))+"/knowledge/";
 ...
 */
function getRiskModel(ChartID, gradata, showID) {
	var gdata=JSON.parse(gradata)
	if(flag){
		creatDiv(ChartID, showID);
		flag=false;
	}
	
	//用于使chart自适应高度和宽度,通过窗体高宽计算容器高宽
	var resizemyChart1 = function() {
		document.getElementById('modelChart').style.width = window.innerWidth / 2 + 'px';
		document.getElementById('riskChart').style.width = window.innerWidth / 2 + 'px';
	};
	//设置容器高宽
	resizemyChart1();

	var ModelChart = echarts.init(document.getElementById('modelChart'));
	...
	var tablecont = "<label>姓名：" + gdata.gName + "</label><table id='riskfactor-table'><tr><td>序号</td><td>出现时间</td><td>风险因素</td> <td>因素来源</td><td>风险分值</td></tr></table>"
	$('#' + showID+" .riskfactor-Msg").html(tablecont);
	var risk = gdata.risklist;
	...
	if(advice != null && advice.length > 0) {
		...
		$('#' + showID+" .riskfactor-Advice").append(tipcont)
	}

	/***************     计算各项风险得分         *****************/
	var data = gdata.risklist;
	//console.log(data)
	...
	var content = "<p style='margin-top:10px;color:#FD8A5F;'>本次风险评估总分=" + sumScore + "分</p>" +
		"<p style='color:#888888;width:170%;'>风险总分=化验检查得分*20%+基因检查得分*20%+症状得分*20%+高危自评得分*20%+自测得分*20%</p>" +
		"<p style='color:#888888;width:170%;'>风险总分：2~4分属于轻度高危；5~7分属于中度高危；8分以上属于重度高危</p>"

	$('.riskfactor-Msg').append(content)
...
//生成所需的DIV
function creatDiv(ChartID, showID){
	$('#'+ChartID).append("<div id='modelChart'></div><div id='riskChart'></div>")
	
	var divCont="<div class='row'><div class='col-md-6'><label>风险因素:</label><div class='riskfactor-Msg'>"+
     "</div></div><div class='col-md-12'><label>评估建议:</label><div class='riskfactor-Advice'></div></div></div>";
