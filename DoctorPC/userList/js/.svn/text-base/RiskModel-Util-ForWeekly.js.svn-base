			...
			if(result != null || result != '') {
				...
				$.ajax({
					...
					success: function(e) {
						...
						$("#abnormal-analysis").css("padding","10px");
					}
				});
			}
		}
...
function getRiskModel(ChartID, gradata, showID) {
	...
	var resizemyChart1 = function() {
		document.getElementById('modelChart').style.width = window.innerWidth + 'px';
		document.getElementById('riskChart').style.width = window.innerWidth + 'px';
	};
	//设置容器高宽
	resizemyChart1();
	...
	/*********  指标各项异常信息    **********/
	var tablecont = "<div class='historyTableContainer'><table id='riskfactor-table'><tr><td>序号</td><td>出现时间</td><td>风险因素</td> <td>因素来源</td><td>风险分值</td></tr></table></div>"
