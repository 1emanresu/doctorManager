...
var resizemyChart8 = function() {
	chart8.style.width = 1000 / 2 - 20 + 'px';
};
//设置容器高宽
resizemyChart8();
// 基于准备好的dom，初始化echarts实例
			...
			$.each(list, function(i) {
				...
				/*if(reportName.trim()=='血常规'){
					expecialCheckStr+="<div class='class_firstDiagnose_bloodRule_content'>"
									  ...
									  +"<td>名称</td><td>简称</td><td>结果</td><td>单位</td><td>参考值</td></tr>";
					$.each(checkDetailList, function(j) {
						var shortname='';
						if(checkDetailList[j].Checkindex.shortname!=null){
							shortname=checkDetailList[j].Checkindex.shortname;
						}
						var checkresult='';
					...
					if(list[i].picLink!=null&&list[i].picLink.trim()!=''){
						src=sessionStorage.getItem("address").split("doctormanager")[0]+"gravidamanager/pics/report/"+list[i].picLink+".jpg";
					}
					expecialCheckStr+="<tr class='tr_firstDiagnose_width'><td></td><td></td><td></td><td></td><td>"
				...
				/*}else if(reportName.trim()=='尿常规'){
					expecialCheckStr+="<div class='class_firstDiagnose_bloodRule_content'>"
									  ...
									  +"<table>"
									  +"<tr class='tr_firstDiagnose_width tr_firstDiagnose_color'>"
									  +"<td>名称</td><td>结果</td><td>参考值</td><td>名称</td><td>结果</td><td>参考值</td></tr>";
					$.each(checkDetailList, function(j) {
						var checkresult='';
						if(checkDetailList[j].checkvalue!=null){
							checkresult=checkDetailList[j].checkvalue;
						...
						if(j==checkDetailList.list-1&&j%2==0){
							expecialCheckStr+="<td></td><td></td><td></td>";
						}
					});
					var src=''
					if(list[i].picLink!=null&&list[i].picLink.trim()!=''){
					...
					expecialCheckStr+="<tr class='tr_firstDiagnose_width'><td></td><td></td><td></td><td></td><td></td><td>"
									  +"<a style='cursor:pointer;' onclick='seeOriginalReport("
									  +src 
									  +")>查看原始报告单</a>"
									  +"</td></tr></table></div></div>"
...
$("#minute").html(min);
var seconds=59;
var a=setInterval(countDown,1000);//1000毫秒
function countDown(){
	if(seconds==-1){
		if(min==0){
		clearInterval(a);
