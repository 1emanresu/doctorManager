(function(w) {
	w.Calendar = function(opt) {
		// 创建日历控件基本结构	
		var cldbox = document.getElementById("foo_toobox_mediRecord");
		cldbox.className = 'calendar-container';
		var tpl = "";
		tpl += '<div class="calendar-title">';
		tpl += '<div class="calendar-prevyear"><img src="images/zuoshuang.png" width="14px"></div>';
		tpl += '<div class="calendar-prevmonth"><img src="images/zuo.png" width="17px"></div>';
		tpl += '<div class="calendar-year"></div>';
		tpl += '<div class="calendar-month"></div>';
		tpl += '<div class="calendar-nextmonth"><img src="images/you.png" width="17px"></div>';
		tpl += '<div class="calendar-nextyear"><img src="images/youshuang.png" width="14px"></div>';
		tpl += '</div>';
		...
		//初始化显示本月信息
		setContent(content, fistWeek, monDaynum, dateObj);
	...
	function setContent(el, fistWeek, monDaynum, nowDay) {
		...
		for(var i = 1; i <= monDaynum; i++) {
			...
			if(i == nowDay.getDate() && date.getFullYear() == nowDay.getFullYear() && date.getMonth() == nowDay.getMonth()) {
