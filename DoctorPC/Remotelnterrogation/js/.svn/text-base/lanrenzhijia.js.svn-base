...
function huabi() {
	...
	$(canvas2).bind('mousedown', function(event) {
		drawable = true;
		origin.x = event.clientX - $("#canvas").offset().left;
		origin.y = event.clientY - $("#canvas").offset().top;
	...
	$(canvas2).bind('mouseup', function(event) {
		if((type == 1 || type == 3 || type == 4) && drawable == true) {
			...
			end.x = event.clientX - $("#canvas").offset().left;
			end.y = event.clientY - $("#canvas").offset().top;
			draw(context);
		} else
			drawable = false;
		color_changeable = false;
