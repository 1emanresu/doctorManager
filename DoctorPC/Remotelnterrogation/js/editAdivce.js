...
//				}
				...
				function sendAdviceMsg() {
					...
					//定义发送的数据
					var data=new Date();			
					$('#chat_content').append("<div class='sendMsg_div'>"+data.getHours()+":"+data.getMinutes()+"</div>")
					var send = "<div class='sendMsg_div'><div class='sendMsg'>" + cont + "</div></div>";
					$('#chat_content').append(send)
					//设置最底滚动条
				...
				function showTipsItem(data) {
					...
					$('#tips').append("<button class='btn btn-default'>"+page.index+"</button>");
	...
	var edittips = new EditTips("sendMessageTextArea");
	edittips.init();
	util=edittips;
