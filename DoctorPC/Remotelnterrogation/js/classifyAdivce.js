...
$(function() {
	...
	function init() {
		...
		obj = {
			...
			"tip_box": "#tips",
			//医嘱编辑框
			"advice_box": "#sendMessageTextArea",
			//关键字集合
			"keyworks": ".advice",
			//关键字的属性的定义
			"attr": "keyword",
	...
	$(obj.data_display).on("dblclick", "p", function() {
		var beforeText = getText(document.getElementById("sendMessageTextArea"));
		var afterText = $(this).text();
		$(obj.advice_box).text("");
		insert_advice_box("sendMessageTextArea", beforeText + afterText);
		//用户登记
		var id = $(this).attr("id");
	...
	function pageShow() {
		...
		for(var i = index;
			...
			if(data[i].searchid != undefined || data[i].searchid != null){
				var id = data[i].searchid;
			}
			
			var a = data[i].value;
			$(obj.data_display).append("<p style='margin-top:0px!important' id='" + id + "'>" + a + "</p>");
		}
	}
	//点击打开（添加键值对模态框）
	...
	$(obj.adopt_btn).click(function() {
		...
		insert_advice_box("sendMessageTextArea", str);
	});
	//去除<标><非标>
	...
	$("#tools_bar img").click(function() {
		...
		var $div = $(".class_serious_risk1");
		$($div[index]).show();
	});

	$(".img_close2,.img_close").click(function() {
		$(this).parent().hide();
	});
	...
	$(function() {
		var drag_divs = $(".class_serious_risk1");
