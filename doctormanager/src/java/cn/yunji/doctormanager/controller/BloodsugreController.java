...
@Controller
@RequestMapping("/bloodsugre")
public class BloodsugreController{
	@Resource
	...
	public JsonResult find(String mid){
		try {
			List<Bloodsugre> list=bloodsugreService.find(mid);
	...
	public JsonResult findByWeekly(String mid,Integer eWeeks,Integer sWeeks){
		...
		try {
			List<Bloodsugre> list=bloodsugreService.findByDateRange(mid, begin, end);
			return new JsonResult(list);
		}catch (Exception e) {
			return new JsonResult(e);
		}	
    ...
    public JsonResult findByDateRange2(String mid,String endDate) throws ParseException{
        ...
        List<Bloodsugre> list=bloodsugreService.findByDateRange(mid, begin, endDate2);
        return new JsonResult(list);    
    }
}
