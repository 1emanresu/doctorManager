...
public class BloodoxygenController{
	...
	public JsonResult find(String mid){
		try {
			List<Bloodoxygen> list=bloodoxygenService.findByMid(mid);
	...
	public JsonResult findByWeekly(String mid,Integer eWeeks,Integer sWeeks){
		...
		try {
			List<Bloodoxygen> list=bloodoxygenService.findByDateRange(mid, begin, end);
			return new JsonResult(list);
		}catch (Exception e) {
			return new JsonResult(e);
		}	
    ...
    public JsonResult findByDateRange2(String mid,String endDate) throws ParseException{
        ...
        List<Bloodoxygen> list=bloodoxygenService.findByDateRange(mid, begin, endDate2);
        return new JsonResult(list);    
    }
}
